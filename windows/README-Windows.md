...
- 点击周配额区域：打开当前会话上下文容量与本机 Codex Token 使用详情。
- 点击上下文容量区域：进入“容量结构 / 项目占比 / 对话明细”。
...
- 上下文容量读取当前活动会话最新 `token_count` 中的 `model_context_window` 与输入明细。
- 项目和对话明细读取 `session_meta` 的会话 ID 与 `cwd`，并关联每个会话的数值 Token 累计。
- Token 详情只统计本机 `.codex` 会话中的数字事件。
- 不保存访问令牌、提示词、聊天正文或原始配额响应。

上下文占用率按最新请求的 `input_tokens ÷ model_context_window` 计算。缓存复用属于输入子集，推理属于输出子集；界面不会重复相加。剩余低于 50% 时提示整理无关上下文，低于 10% 时提示总结并新建任务。项目路径和会话 ID 只在内存中即时解析，不写入缓存；程序不读取消息正文为对话命名。
