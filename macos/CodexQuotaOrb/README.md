...
- “累计上下文用量”区域可点击，进入“上下文结构 / 项目占比 / 对话明细”；“返回”或 `Esc` 回到上一层。
...
| 中间累计上下文 | 所有本地会话的数值型 `input_tokens` 增量 | 累计输入、缓存复用、新增输入、项目数与对话数 |
| 项目 / 对话占比 | `session_meta` 的会话 ID、`cwd` 与每个会话的累计上下文输入 | 每个本机项目和独立对话的累计上下文占用量 |
| Token 详情页 | `~/.codex/sessions` 与 `~/.codex/archived_sessions` | 本机历史消耗统计 |

这些路径不会混用。上方统计卡与下方活动图负责本机 Token 历史，中间卡汇总所有本地对话的累计上下文输入，并且不显示当前活动对话的单次容量、月重置时间或账户版本。每个会话优先使用 `total_token_usage` 的累计输入分项计算增量；旧记录缺少累计分项时，才累加 `last_token_usage` 数值分项。累计计数重置后按新段继续累计。项目页和对话页的“累计占比”都以全部累计上下文输入为分母。缓存复用属于输入子集，推理属于输出子集，界面不会重复相加。项目路径和会话 ID 只在内存中即时解析，不写入缓存，也不读取消息正文为对话命名。本机 Token 历史绝不用于推算官方配额。程序不会保存访问令牌、账户 ID、原始配额响应、日志内容、提示词或聊天内容。
