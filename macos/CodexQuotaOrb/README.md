...
- “当前会话上下文”区域可点击，进入“容量结构 / 项目占比 / 对话明细”；“返回”或 `Esc` 回到上一层。
...
| 中间当前会话上下文 | Codex Desktop 前台活动会话最新 `token_count` 数值事件 | 输入占用、剩余容量、缓存复用、新增输入、输出、推理与原始采样时间 |
