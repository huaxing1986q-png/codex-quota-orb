...
这些路径不会混用。上下文占用率通常按最新请求的 `input_tokens ÷ model_context_window` 计算；Codex 压缩边界记录若出现输入分项为零、但本轮总量仍为正，则按“本轮总量减去输出”恢复压缩后的输入占用，避免错误显示 `0%`，且绝不使用会话累计 Token 代替上下文。缓存复用属于输入子集，推理属于输出子集。剩余低于 50% 时提示整理无关上下文，低于 10% 时提示总结并新建任务。项目路径和会话 ID 只在内存中即时解析，不写入缓存，也不读取消息正文为对话命名。本机 Token 历史绝不用于推算官方配额。程序不会保存访问令牌、账户 ID、原始配额响应、提示词或聊天内容。
