...
- “本月总用量”区域可点击，进入“容量结构 / 项目占比 / 对话明细”；“返回”或 `Esc` 回到上一层。
...
| 详情页本月总用量 | 本机数值型 `token_count` 历史 | 本月 Token、占本机累计总量比例、本机累计及当前项目/对话占比 |
| 当前上下文容量 | Codex Desktop 前台活动会话最新 `token_count` 数值事件 | 输入占用、剩余容量、缓存复用、新增输入、输出、推理与原始采样时间 |
...
这些路径不会混用。本月用量按当前自然月累计，但详情卡不显示月重置时间或账户版本。前台会话 ID 优先从 Codex Desktop 本机日志的活动路由行中即时提取，日志不可用时按最近一次非压缩输入活动回退。上下文占用率通常按最新请求的 `input_tokens ÷ model_context_window` 计算；Codex 压缩边界记录若出现输入分项为零、但本轮总量仍为正，则按“本轮总量减去输出”恢复压缩后的输入占用，避免错误显示 `0%`，且绝不使用会话累计 Token 代替上下文。压缩记录无法可靠拆分缓存与新增输入时，两项显示为 `—`。缓存复用属于输入子集，推理属于输出子集。剩余低于 50% 时提示整理无关上下文，低于 10% 时提示总结并新建任务。项目路径、会话 ID 和前台路由 ID 只在内存中即时解析，不写入缓存，也不读取消息正文为对话命名。本机 Token 历史绝不用于推算官方配额。程序不会保存访问令牌、账户 ID、原始配额响应、日志内容、提示词或聊天内容。
