...
description: Operate the native macOS or Windows Codex Quota Orb, including its authoritative weekly quota display, cumulative local context usage, draggable high-DPI floating window, and local Codex token-history details page.
---
...
The details page keeps local Token history in the top metric row and bottom activity area: local cumulative, today, current month, current week, daily heatmap, weekly trend, and cumulative trend. The middle card must show cumulative context input across every readable local Codex conversation, never the currently active conversation and never a single `model_context_window`. Do not show remaining capacity, a monthly reset time, or an account plan in this card.

Build cumulative context from numeric `token_count` events in every local session. For each conversation, prefer monotonic deltas of `total_token_usage.input_tokens`; when cumulative component counters are absent, sum numeric `last_token_usage.input_tokens`. If a cumulative counter resets, treat the new value as a new segment instead of subtracting across the reset. Aggregate cached input, output, and reasoning output by the same rule. Cached input is a subset of context input, fresh input is `input - cached input`, and reasoning output is a subset of output; never add those subsets twice. The main context card shows cumulative input, its share of local cumulative Token history, cached reuse, fresh input, project count, conversation count, and update time. Output and reasoning are non-context indicators and stay explicitly labeled in the next-level structure page.

Click the full cumulative-context region to open the next-level context and usage page. It has three views: cached-versus-fresh cumulative context structure; per-project share grouped by the `session_meta.cwd` path; and every conversation sorted by cumulative context input. Both project and conversation percentages use total cumulative context input as the denominator, so both levels reconcile to 100%. Conversation labels use date/time plus a short session ID and never derive a title from message text. The page must include a text `Back` / `返回` control; `Esc` also returns to the parent details page. Project paths and session IDs are parsed in memory and must not be written to the numeric history cache.
...
- Keep cumulative context separate from account quota and from total Token history. Cumulative context is the sum of input-token increments across all local conversations; it has no global capacity or reset time.
- Never use the foreground conversation, `model_context_window`, or current-session occupancy in the cumulative context card.
