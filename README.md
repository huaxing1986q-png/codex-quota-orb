...
- **职责分区清晰**：上方统计卡和下方活动图显示本机 Token 历史；中间卡只显示当前活动会话的上下文容量。
...
| 当前会话上下文入口 | 下一层容量结构 |
| --- | --- |
| ![Current session context](docs/images/context-capacity-windows-preview.png) | ![Capacity and usage details](docs/images/context-breakdown-windows-preview.png) |
...
详情页上方四张统计卡与下方 Token 活动图继续显示本机累计、今日、本月、本周及历史趋势。中间信息区只显示当前活动会话上下文：占用百分比、输入、总容量、剩余容量、缓存复用、新增输入和上下文采样时间。这里不显示月重置时间、账户版本、本机累计或会话累计 Token。官方周配额继续只在浮球与周配额卡中显示。
...
5. 点击当前会话上下文区域：进入容量结构、项目占比和对话明细；“返回”或 `Esc` 回到上一层。
6. 鼠标移出卡片或点击桌面：自动回缩。
...
| 中间当前会话上下文 | 当前活动会话最新 `token_count` 数值事件 | 最新请求输入占用、模型上下文容量、剩余及输入结构明细 |
