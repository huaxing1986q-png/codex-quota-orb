...
- **本月总用量主指标**：详情页主区显示本月 Token、占本机累计历史的比例、本机累计，以及当前项目和当前对话的总占比。
...
| 本月总用量入口 | 下一层容量结构 |
| --- | --- |
| ![Monthly total usage](docs/images/context-capacity-windows-preview.png) | ![Capacity and usage details](docs/images/context-breakdown-windows-preview.png) |
...
详情页主信息区只使用本机 Token 历史：显示本月 Token、本月占本机累计总量的比例、当前项目总占比、当前对话总占比、对话在项目内占比和本机累计。这里不显示月重置时间或账户版本。官方周配额继续只在浮球与周配额卡中显示；当前上下文容量保留在下一层容量明细中。
...
点击详情页中的整块“本月总用量”区域，会进入下一层“容量与占用明细”：
...
5. 点击本月总用量区域：进入容量结构、项目占比和对话明细；“返回”或 `Esc` 回到上一层。
6. 鼠标移出卡片或点击桌面：自动回缩。
